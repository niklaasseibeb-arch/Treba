...
export default function LogDriverContactDialog({ open, onOpenChange, tripRequestId, onLogged }) {
  ...
  const submit = async () => {
    setBusy(true);
    try {
      const res = await base44.functions.invoke("logPassengerContactAttempt", {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader><DialogTitle>Log contact attempt to driver</DialogTitle></DialogHeader>
