...
import { Loader2, ShieldAlert, Clock, Scale } from "lucide-react";
...
export default function AdminNoShowCases() {
  ...
  const [policyForm, setPolicyForm] = useState({ name: "Default", grace_minutes: 10, dispute_window_hours: 48, refund_enabled: false, refund_percentage: 0, charge_passenger: true, is_active: true, description: "" });
  const [policyId, setPolicyId] = useState(null);
  const [savingPolicy, setSavingPolicy] = useState(false);

  const load = async () => {
    try {
      const list = await base44.entities.NoShowReport.list("-reported_at", 200);
      setReports(list || []);
      ...
      if (active) {
        ...
        setPolicyForm({
          ...
          grace_minutes: active.grace_minutes ?? 10,
          dispute_window_hours: active.dispute_window_hours ?? 48,
          refund_enabled: !!active.refund_enabled,
          refund_percentage: active.refund_percentage ?? 0,
          charge_passenger: !!active.charge_passenger,
  ...
  const openCase = async (r) => {
    ...
    setCaseData(null);
    try {
      const res = await base44.functions.invoke("getNoShowCase", { no_show_report_id: r.id });
  ...
  const submitReview = async () => {
    setBusy(true);
    try {
      const res = await base44.functions.invoke("reviewNoShow", { no_show_report_id: selected, decision: review.decision, review_notes: review.notes, refund_amount: review.refund_amount });
      if (res.data?.error) { toast({ title: res.data.error, variant: "destructive" }); return; }
      toast({ title: `No-show ${res.data.status}` });
  ...
  const savePolicy = async () => {
    ...
    try {
      const payload = {
        name: policyForm.name.trim() || "Default",
        ...
        charge_passenger: !!policyForm.charge_passenger,
        is_active: !!policyForm.is_active,
        description: policyForm.description,
      };
      if (policyId) await base44.entities.NoShowPolicy.update(policyId, payload);
      else { const c = await base44.entities.NoShowPolicy.create(payload); setPolicyId(c.id); }
      toast({ title: "No-show policy saved" });
      load();
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="flex items-center gap-2 text-2xl font-bold tracking-tight"><ShieldAlert className="h-6 w-6 text-primary" /> No-Show Cases</h1>
        <p className="mt-1 text-muted-foreground">Review driver no-show reports and passenger disputes. Treba never auto-refunds — financial outcomes are applied per policy.</p>
      </div>

      <div className="rounded-2xl border border-border bg-card p-5 treba-shadow">
        <div className="flex items-center gap-2 text-sm font-semibold"><Scale className="h-4 w-4 text-primary" /> No-show policy</div>
        <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div className="space-y-1.5"><Label>Grace minutes</Label><Input type="number" value={policyForm.grace_minutes} onChange={(e) => setPolicyForm({ ...policyForm, grace_minutes: e.target.value })} className="h-10" /></div>
          <div className="space-y-1.5"><Label>Dispute window (hours)</Label><Input type="number" value={policyForm.dispute_window_hours} onChange={(e) => setPolicyForm({ ...policyForm, dispute_window_hours: e.target.value })} className="h-10" /></div>
          ...
          <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={policyForm.charge_passenger} onChange={(e) => setPolicyForm({ ...policyForm, charge_passenger: e.target.checked })} /> Charge passenger</label>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="space-y-3">
          {reports.length === 0 ? <p className="text-sm text-muted-foreground">No no-show reports yet.</p> : reports.map((r) => (
            <button key={r.id} onClick={() => openCase(r)} className={`w-full rounded-xl border p-4 text-left ${selected === r.id ? "border-primary bg-primary/5" : "border-border bg-card"}`}>
              <div className="flex items-center justify-between">
                <span className="text-sm font-semibold">{r.passenger_name || "Passenger"}</span>
                <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${STATUS_TONE[r.no_show_status] || STATUS_TONE.reported}`}>{r.no_show_status}</span>
              </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-5 treba-shadow">
          {!caseData ? <p className="text-sm text-muted-foreground">Select a case to review.</p> : (
            <div className="space-y-4">
              ...
              {caseData.report.dispute_reason && <div className="text-xs text-muted-foreground">Dispute: {caseData.report.dispute_reason}</div>}
              <div>
              ...
              {["disputed", "reported"].includes(caseData.report.no_show_status) && (
                <div className="space-y-3 rounded-xl border border-border p-3">
                  <div className="text-sm font-semibold">Review decision</div>
                  <div className="flex gap-2">
                    <Button size="sm" variant="destructive" onClick={() => setReview({ ...review, decision: "upheld" })} className={review.decision === "upheld" ? "ring-2 ring-offset-1" : ""}>Uphold</Button>
